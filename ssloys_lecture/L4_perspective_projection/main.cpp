...
void triangle(vec3 *vertices, vec3 *texturecoord, double *zbuffer, TGAImage *image, TGAImage &diffusemap, float strength)
{
   ...
   for (double x = boundmin.x; x < boundmax.x; x++)
   {
      for (double y = boundmin.y; y < boundmax.y; y++)
      {
         ...
         // z buffer must base on camera location. Right now, change to -z if camera is point from behind
         if(camera_coord[2] < 0) z = -z;


         int index = x + y * imgw;
         if (barycentricvec.x < 0 || barycentricvec.y < 0 || barycentricvec.z < 0 || zbuffer[index] >= z)
            continue;
...
int main(int argc, char **argv)
{
   ...
   for (int i = 0; i < model->nfaces(); i++)
   {
      ...
      if (strength > 0)
      {
         triangle(screencoord, texturecoord, zbuffer, image, diffusemap, strength);
      }
   }
