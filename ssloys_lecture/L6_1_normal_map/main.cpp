...
void triangle(vec3 *vertices, vec3 *texturecoord, vec3 *normvec, double *zbuffer, TGAImage *image, TGAImage &diffusemap,const Model& model)
{
   ...
   for (double x = boundmin.x; x < boundmax.x; x++)
   {
      for (double y = boundmin.y; y < boundmax.y; y++)
      {
         ...
         // shadingvector = shadingvector * contrast;

         // raise contrast base on contrast
         shadingvector = model.normal(colorcoord) * contrast;
         ...
         double strength = (0 - shadingvector * lightsource_direction.normalized()) * intensity;
...
int main(int argc, char **argv)
{
   ...
   mat<4, 4> Viewport = viewport(imagew / 8, imageh / 8, imagew * 3 / 4, imageh * 3 / 4);
   ...
   for (int i = 0; i < model->nfaces(); i++)
   {
      ...
      triangle(vertices, texvertices, normvecs, zbuffer, image, diffusemap, *model);
   }
